...
export class IntelligentGoalManager {
  ...
  generateRecommendation(
    ...
    const averagePerformance =
      recentPerformance.reduce((sum, val) => sum + val, 0) /
      recentPerformance.length;
    const achievementRate =
      recentPerformance.filter((val) => val >= currentGoal).length /
      recentPerformance.length;

    // Calculate performance consistency
    const variance =
      recentPerformance.reduce(
        (sum, val) => sum + Math.pow(val - averagePerformance, 2),
        0
      ) / recentPerformance.length;
    ...
    if (
      ...
      achievementRate < 0.4 &&
      averagePerformance < currentGoal * 0.8
    ) {
      // Struggling with current goal - decrease
      const decreasePercentage = Math.min(
        0.3,
        (currentGoal - averagePerformance) / currentGoal
      );
      recommendedGoal = Math.round(
        currentGoal * (1 - decreasePercentage * 0.7)
      ); // More conservative decrease
      reasoning = `Goal may be too challenging (${Math.round(
        achievementRate * 100
      )}% success rate)`;
      ...
      reasoning = `Current goal provides good challenge (${Math.round(
        achievementRate * 100
      )}% success rate)`;
      adjustmentType = "maintain";
      confidence = 0.7;
