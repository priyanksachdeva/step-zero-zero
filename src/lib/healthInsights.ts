...
export class HealthInsightsEngine {
   ...
   */
  analyzeActivityType(
    stepData: { steps: number; timestamp: Date }[]
  ): "walking" | "running" | "stationary" | "unknown" {
    ...
    for (let i = 1; i < stepData.length; i++) {
      const timeDiff =
        (stepData[i].timestamp.getTime() -
          stepData[i - 1].timestamp.getTime()) /
        (1000 * 60); // minutes
  ...
  processDailyActivity(
    ...
    const totalTime =
      (stepData[stepData.length - 1].timestamp.getTime() -
        stepData[0].timestamp.getTime()) /
      (1000 * 60);
    const restingMinutes = Math.max(0, totalTime - activeMinutes);

    const walkingEfficiency = this.calculateWalkingEfficiency(
      totalSteps,
      totalDistance,
      activeMinutes
    );
  ...
  generateHealthInsights(recentMetrics: HealthMetrics[]): HealthInsight[] {
    ...
    if (latest.averagePace > 0) {
      ...
      if (latest.averagePace < 70) {
        ...
        paceRecommendation =
          "Try to increase pace gradually for better cardiovascular benefits";
    ...
    if (latest.totalSteps > 0) {
      ...
      insights.push({
        ...
        recommendation:
          efficiency === "LOW"
            ? "Try walking at a brisker pace or include inclines"
            : "Great calorie efficiency",
      });
    }

    // Activity consistency
    const activeMinutesPercentage =
      (latest.activeMinutes / (latest.activeMinutes + latest.restingMinutes)) *
      100;
    if (activeMinutesPercentage > 0) {
      insights.push({
        ...
        )}% of tracked time active`,
        value: latest.activeMinutes,
        unit: "minutes",
        recommendation:
          activeMinutesPercentage < 15
            ? "Try to increase active time throughout the day"
            : "Good activity distribution",
      });
    }
    ...
    if (latest.walkingEfficiency > 0) {
      ...
      insights.push({
        ...
        recommendation:
          efficiencyScore < 70
            ? "Focus on consistent pace and stride length"
  ...
  private calculateAverageMetrics(metrics: HealthMetrics[]): HealthMetrics {
    ...
    return {
      ...
      walkingEfficiency:
        Math.round((totals.walkingEfficiency / count) * 100) / 100,
    };
  }
